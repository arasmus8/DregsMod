...
public class AcceptanceParticleEffect extends AbstractGameEffect {
    private float x;
    private float y;
    private float vX;
    private float vY;
    ...
    public AcceptanceParticleEffect() {
        img = ImageMaster.WHITE_RING;
        duration = MathUtils.random(1.3F, 1.8F);
        scale = MathUtils.random(0.1F, 0.2F) * Settings.scale;
        dur_div2 = duration / 2.0F;
        color = new Color(MathUtils.random(0.5F, 1.0F), MathUtils.random(0.6F, 0.9F), 0.0F, 0.0F);
        ...
        vX = MathUtils.random(1.0F, 20.0F);
        vY = MathUtils.random(0.5F, 10.0F);
        if (x < AbstractDungeon.player.hb.cX) {
            vX *= -1;
        }
        if (y < AbstractDungeon.player.hb.cY) {
            vY *= -1;
        }
